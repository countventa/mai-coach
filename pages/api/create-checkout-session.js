...
export default async function handler(req, res){
  try{
    ...
    const baseSuccessUrl = process.env.SUCCESS_URL || 'http://localhost:3000/success';
    const successUrlWithPrice = `${baseSuccessUrl}?price=${encodeURIComponent(priceId)}`;

    const session = await stripe.checkout.sessions.create({
      mode: mode === 'subscription' ? 'subscription' : 'payment',
      line_items: [{ price: priceId, quantity: 1 }],
      success_url: successUrlWithPrice,
